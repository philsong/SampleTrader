...
def readConf(): 
    ...
    try:
        cf.read(os.path.join(rootDir,"cmfchina.conf"))   
        self.sendEMail = cf.get("EMAIL", "TO",'')
        mail_from = cf.get("EMAIL", "FROM",'')        
        mail_server = cf.get("EMAIL", "SERVER",'')
        mail_id = cf.get("EMAIL", "USER",'')        
        mail_pw = cf.get("EMAIL", "PASSWORD",'')        
        mail_title = cf.get("EMAIL", "TITLE",'')       
...
if __name__ == '__main__':
    global logger, rootDir, level 

    __author__ = 'TianJun'
    parser = argparse.ArgumentParser(description='This is a rebate script by TianJun.')
    parser.add_argument('-d','--rootdir', help='Input log file dir,default is "/home/gfqhhk/doc/settlement".',required=False)
